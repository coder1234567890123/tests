...
describe('Auth Controller', () => {
  beforeEach(() => {
     ...
     (sendRegistrationEmail as jest.Mock).mockReset();
     (sendPasswordResetEmail as jest.Mock).mockReset();
  ...
  describe('POST /api/auth/register', () => {
    it('should register a new user successfully and send a registration email', async () => {
      ...
      (sendRegistrationEmail as jest.Mock).mockResolvedValue(undefined);
  ...
  describe('POST /api/auth/login', () => {
     ...
     it('should return 401 for incorrect password', async () => {
        const hashedPassword = await bcrypt.hash('password123', 10);
        (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'user1', email: 'test@example.com', password: hashedPassword, roles: ['USER'], enabled: true });
        const res = await request(app)
            .post('/api/auth/login')
    ...
    it('should return 401 for disabled user', async () => {
        const hashedPassword = await bcrypt.hash('password123', 10);
        (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'user1', email: 'test@example.com', password: hashedPassword, roles: ['USER'], enabled: false });
        const res = await request(app)
            .post('/api/auth/login')
            .send({ email: 'test@example.com', password: 'password123' });
        expect(res.statusCode).toEqual(401);
  ...
  describe('POST /api/auth/request-password-reset', () => {
    it('should return success message and send email if user exists and is enabled', async () => {
      ...
      (prisma.user.update as jest.Mock).mockResolvedValue({}); // Mock update for token
    ...
    it('should return success message if user is disabled (to prevent account status enumeration)', async () => {
        (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'userDisabled', email: 'disabled@example.com', enabled: false });
  ...
  describe('POST /api/auth/reset-password/:token', () => {
    ...
    it('should reset password successfully with a valid token', async () => {
      (prisma.user.findFirst as jest.Mock).mockResolvedValue({ id: 'user1', email: 'reset@example.com', token: validToken, tokenRequested: new Date(Date.now() + 3600000), enabled: true }); // Token not expired
    ...
    it('should return 403 if user account is disabled', async () => {
        (prisma.user.findFirst as jest.Mock).mockResolvedValue({ id: 'user1', email: 'reset@example.com', token: validToken, tokenRequested: new Date(Date.now() + 3600000), enabled: false });
