          ...
          <v-flex m4>
            <v-card>
              <div class="container">
                ...
                <div v-if="currentPackage == 'pre_paid'">
                  <v-layout>
                    <v-flex
                      m2
                      class="padding5">
                ...
                <div v-if="currentPackage == 'suspended'">
                  <v-card class="padding5">
                    <h1>Account Has been Suspended</h1>
                  </v-card>
                </div>
              </div>
            </v-card>
...
export default {
  ...
  watch: {
    companyId: {
      handler: function() {
        this.$store.dispatch('product-types/getCompanyUsage', this.companyId)
      },
      deep: true
    },
    dateFrom(val) {
  ...
  methods: {
    ...
    refreshBundles() {
      this.loading = true
      this.$store
        .dispatch('product-types/getCompanyUsage', this.companyId)
        .then(() => {
          this.loading = false
        })
    },
    ...
    exportQueues() {
      this.loading = true
      this.$store
        .dispatch('product-types/queryExportUsage', this.companyId)
        .then(response => {
